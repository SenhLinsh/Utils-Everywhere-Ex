...
import com.linsh.lshutils.viewholder.LoadMoreFooterHolder;
 ...
 * </pre>
 */
public abstract class LoadMoreFooterRcvAdapter<T, H extends SimpleViewHolder<T>> extends SimpleRcvAdapter<T, SimpleViewHolder<T>> {
    ...
    @Override
    protected SimpleViewHolder<T> getViewHolder(ViewGroup parent, int viewType) {
    ...
    protected abstract LoadMoreFooterHolder getFooterHolder(ViewGroup parent);

    @Override
    protected void onBindViewHolder(SimpleViewHolder<T> holder, T data, int position) {
        ...
        if (holder instanceof LoadMoreFooterHolder) {
            // 如果是 Footer, 则更新状态
            LoadMoreFooterHolder footerHolder = (LoadMoreFooterHolder) holder;
